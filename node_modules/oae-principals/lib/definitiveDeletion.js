                ...
                if(hasRole) {
                    ...
                    ContentAPI.getContentMembersLibrary(ctx, element.id, null, null, function(err, memberList) {
                        ...
                        _deleteManagerArray(memberList, user.id, function(err, newMemberList) {
                            ...
                            newMemberList.forEach(function(person, idx, array) {
                    ...
                    FolderAPI.getFolderMembers(ctx, element.id, null, null, function(err, memberList) {
                        ...
                        _deleteManagerArray(memberList, user.id, function(err, newMemberList) {
                            ...
                            if(_.isEmpty(newMemberList)) { 
                                ...
                                // TODO : Do not work && no err ... wtf ? 
                                FolderAPI.setFolderPermissions(ctx, element.id, update, function(err) {
                                    if(err) {
                                        return callback(err);
                                    }
                                    // AuthzAPI.hasRole(archiveUser.idArchive, element.id,  'manager', function(err, hasRole) {
                                    //     console.log('ici')
                                    //     console.log(update)     
                                    //     console.log('archiveUser.idArchive')
                                    //     console.log(archiveUser.idArchive)
                                    //     console.log(hasRole);});
                                    ...
                                    if(!hasManager) {
                                        ...
                                        members.forEach(function(member, idMember, arrayMember) {
                                            ...
                                            if (idMember === arrayMember.length - 1) { 
                                                ...
                                                _setFolderRoleNoVerif(ctx, element.id, update, function(err) {
                                                    if(err) {
                                                        return callback(err);
                                                    }
...
var _deleteRightDiscussion = function(ctx, del, idArchive, idResource, callback) {
    ...
    if(del) {
        ...
        DiscussionsDAO.deleteDiscussion(idResource, function(err) {
            if (err) {
                return callback(err);
            }
            return callback();
...
var _deleteRightGroup = function(ctx, del, idArchive, idResource, callback) {
    if(del) {
        ...
        var group = {};
        group.id = idResource;
        PrincipalsDAO.deleteGroupFromPrincipals(group, function(err) {
            if(err) {
                return callback(err);
            }
            return callback();
