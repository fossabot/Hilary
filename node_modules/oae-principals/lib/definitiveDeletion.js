...
var _transferRightToContentLoop = function(contentHashes, ctx, user, archiveUser, listEmail, listIdElement, callback) {
                    ...
                    if (_.isEmpty(newMemberList)) { 
                        ...
                        ContentAPI.setContentPermissions(ctx, element.id, update, function() {
                            ...
                            ContentAPI.removeContentFromLibrary(ctx, user.id, element.id, function(err) {
                                ...
                                return _transferRightToContentLoop(contentHashes, ctx, user, archiveUser, listEmail, listIdElement, callback);
                            });
                        });
                    } else {
...
var removeProfilePicture = module.exports.removeProfilePicture = function(ctx, user, callback) {
    PrincipalsDAO.getPrincipal(user.id, function (err, principal) {
        ...
        if (_.isEmpty(principal.picture)) {
            ...
            fs.unlink(split.slice(0, split.length - 1).join("/") + "/files/" + pathSmallPicture[1]);
            fs.unlink(split.slice(0, split.length - 1).join("/") + "/files/" + pathMediumPicture[1]);
            fs.unlink(split.slice(0, split.length - 1).join("/") + "/files/" + pathLargePicture[1]);
