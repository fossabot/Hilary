...
var MeetingAPI = require('oae-jitsi/lib/api.meetings'); // TODO : remove lib/api.meetings
...
var transferDataToArchive = module.exports.transferDataToArchive = function (ctx, user, archiveUser,callback) {
    ...
    _transferRightToContent(ctx, user, archiveUser, listEmail, listIdElement, function(err, listEmail, listIdElement) {
        if (err) {
            return callback(err);
        }

        // Transfer or update rights in folders
        _transferRightToFolder(ctx, user, archiveUser, listEmail, listIdElement, function(err, listEmail, listIdElement) {
            if (err) {
                return callback(err);
            } 

            // Transfer or update rights in discussions
            _transferRightToDiscussions(ctx, user, archiveUser, listEmail, listIdElement, function(err, listEmail, listIdElement) {
                if (err) {
                    return callback(err);
                }  

                // Transfer or update rights in discussions
                _transferRightToMeetings(ctx, user, archiveUser, listEmail, listIdElement, function(err, listEmail, listIdElement) {
                    if (err) {
                        return callback(err);
                    }  

                    // Transfer or update rights in groups
                    _transferRightToGroup(ctx, user, archiveUser, listEmail, listIdElement, function(err, listEmail, listIdElement) {
                        if (err) {
                            return callback(err);
                        }

                        // Add element to archive
                        _addToArchive(archiveUser.archiveId, user.id, listIdElement, function(err) {
                            if(err) {
                                return callback(err);
                            }
...
//         // Recursive loop
//         _transferRightToContentLoop(contents, ctx, user, archiveUser, listEmail, function(err, listEmail) {
//             if (err) {
//                 return callback(err);
//             }
//             return callback(null, listEmail);
...
// var _transferRightToContentLoop = function(contentHashes, ctx, user, archiveUser, listEmail, callback) {

//     contentHashes = contentHashes || [];

//     // Return when there are no more folder in the array
//     if (_.isEmpty(contentHashes)) {
//         return callback(null, listEmail);
//     }
...
//                                     return _transferRightToContentLoop(contentHashes, ctx, user, archiveUser, listEmail, callback);
//                                 });
//                             });
//                         }); 
//                     } 
...
var _transferRightToFolderLoop = function(folderHashes, ctx, user, archiveUser, listEmail, listIdElement, callback) {
                        ...
                        AuthzAPI.updateRoles(element.groupId, update, function(err) {
                            ...
                            AuthzAPI.updateRoles(element.id, update, function(err) {
                                ...
                                FolderAPI.removeFolderFromLibrary(ctx, user.id, element.id, function(err) {
                                    // Recursive step inside the callback
                                    return _transferRightToFolderLoop(folderHashes, ctx, user, archiveUser, listEmail, listIdElement, callback);
                                });
                            });
                        });  
                    } else {
...
var _transferRightToDiscussionsLoop = function(discussionHashes, ctx, user, archiveUser, listEmail, listIdElement, callback) {
                    ...
                    if (_.isEmpty(newMemberList)) { 
                        ...
                        DiscussionAPI.setDiscussionPermissions(ctx, element.id, update, function(err) {
                            ...
                            DiscussionAPI.removeDiscussionFromLibrary(ctx, user.id, element.id, function(err) {
                                ...
                                // Recursive step inside the callback
                                return _transferRightToDiscussionsLoop(discussionHashes, ctx, user, archiveUser, listEmail, listIdElement, callback); 
                            });
                        }); 
                    } else {
...
var _transferRightToMeetingsLoop = function(meetingHashes, ctx, user, archiveUser, listEmail, listIdElement, callback) {
                    ...
                    if (_.isEmpty(newMemberList)) { 
                        ...
                        _setMeetingMembersNoVerif (ctx, element, update, function(err) {
                            ...
                            MeetingAPI.removeMeetingFromLibrary(ctx, user.id, element.id, function(err) {
                                ...
                                // Recursive step inside the callback
                                return _transferRightToMeetingsLoop(meetingHashes, ctx, user, archiveUser, listEmail, listIdElement, callback);
                            });
                        });
                    } else {
...
var _transferRightToGroupLoop = function(groupHashes, ctx, user, archiveUser, listEmail, listIdElement, callback) {
                        ...
                        AuthzAPI.updateRoles(element.id, update, function(err) {
                            ...
                            _setGroupMembersNoVerif (ctx, element, update, function(err) {
                                ...
                                AuthzAPI.updateRoles(element.id, update, function(err) {
                                    ...
                                    // Recursive step inside the callback
                                    return _transferRightToGroupLoop(groupHashes, ctx, user, archiveUser, listEmail, listIdElement, callback);
                                });
                            });
                        });
                    } else {
...
var _deleteFollowing = function(ctx, user, callback) {
    ...
    FollowingDAO.getFollowing(user.id, null, null, function(err, userIdsFollowing) {
        ...
        FollowingDAO.deleteFollows(user.id, userIdsFollowing, function(err) {
            ...
            var finish = 0;
            userIdsFollowing.forEach(function(id, i, arr) {
...
var _deleteFollowers = function(ctx, user, callback) {
    ...
    FollowingDAO.getFollowers(user.id, null, null, function(err, userIdsFollowers) {
        ...
        var finish = 0;
        userIdsFollowers.forEach(function(id, i, arr) {
