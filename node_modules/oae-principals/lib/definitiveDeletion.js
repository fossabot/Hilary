...
var MeetingAPI = require('oae-jitsi/lib/api.meetings'); // TODO : remove
...
var getOrCreateArchiveUser =  module.exports.getOrCreateArchiveUser = function(ctx, user, callback) {
    ...
    PrincipalsDAO.getUserArchive(user.tenant.alias, function(err, userArchive) {
        if (err) {
            ...
            var loginId = new LoginId(user.tenant.alias, AuthenticationConstants.providers.LOCAL, user.tenant.alias + 'Archive', {'password': 'password'});
...
var transferDataToArchive = module.exports.transferDataToArchive = function (ctx, user, archiveUser,callback) {
    ...
    _transferRightToContent(ctx, user, archiveUser, listEmail, function(err, listEmail) {
        if (err) {
            return callback(err);
        }

        // Transfer or update rights in folders
        _transferRightToFolder(ctx, user, archiveUser, listEmail, function(err, listEmail) {
            if (err) {
                return callback(err);
            } 

            // Transfer or update rights in discussions
            _transferRightToDiscussions(ctx, user, archiveUser, listEmail, function(err, listEmail) {
                if (err) {
                    return callback(err);
                }  

                // Transfer or update rights in discussions
                _transferRightToMeetings(ctx, user, archiveUser, listEmail, function(err, listEmail) {
                    if (err) {
                        return callback(err);
                    }  

                    // Transfer or update rights in groups
                    _transferRightToGroup(ctx, user, archiveUser, listEmail, function(err, listEmail) {
                        if (err) {
                            return callback(err);
                        }
 ...
 * @api private
 */
var _transferRightToContent = function(ctx, user, archiveUser, listEmail, callback) {
    ...
    ContentAPI.getContentLibraryItems(ctx, user.id, null, null, function(err, contents) {
        ...
        contents.forEach(function(element, id, arr) {
            ...
            ContentAPI.getContentMembersLibrary(ctx, element.id, null, null, function(err, memberList) {
                ...
                // Get his role
                _doesUserHaveRoleManager(user.id, element.id, function(err, hasRole) {
                    if (err) {
                        return callback(err, listEmail);
                    }

                    // Remove the deleted user from the member list
                    _removeDeletedUserToArray(memberList, user.id, function(err, newMemberList) {
                        if (err) {
                            return callback(err, listEmail);
                        }
                        ...
                        if (hasRole) {
                            ...
                            if (_.isEmpty(newMemberList)) { 
                                _addToArchive(archiveUser.archiveId, user.id, element.id, function(err) {
                                    if (err) {
                                        return callback(err, listEmail);
                                    }
                                ...
                                // If it's the last iteration and we don't find any manager, promote all editor -> manager
                                    ...
                                    // If it's the last iteration and we don't have manager(s), give to user archive -> manager role
                                        ...
                                        editors.forEach(function(editor, idEditor, arrayEditor) {
                                            ...
                                            listEmail.push({'userDeleted': editor, 'resource': element, 'memberList': newMemberList, 'action': true});
...
var _parseData = function(data, idUserDeleted, callback) {
    ...
    listElementByMember.users.push({'userWillBePromoted': [], 'contentWillBeDeleted': [], 'userJustLeaving': [], 'profile': []});
    ...
    for (i=0; i<=data.length; i++) { 
        ...
        for (k=0; k<data[i].memberList.length; k++) { 
            exist=false;

            // Create another list to sort by user the list of resource
            for (j=0; j<listElementByMember.users.length; j++) { 
...
var _sendEmail = function(ctx, data, callback) {
    ...
    var userDeleted = {};
    userDeleted = data[0].userDeleted;
    ...
    _parseData(data, userDeleted.id, function(err, parseData) {
        ...
        parseData.users.forEach(function(user, i, arrI) {
 ...
 * @api private
 */
var _transferRightToFolder = function(ctx, user, archiveUser, listEmail, callback) {
    ...
    FolderAPI.getFoldersLibrary(ctx, user.id, null, null, function(err, folders) {
        ...
        // Recursive loop
        _transferRightToFolderLoop(folders, ctx, user, archiveUser, listEmail, function(err, listEmail) {
            if (err) {
                return callback(err);
            }
            return callback(null, listEmail);
...
var _transferRightToFolderLoop = function(folderHashes, ctx, user, archiveUser, listEmail, callback) {

    folderHashes = folderHashes || [];

    // Return when there are no more folder in the array
    if (_.isEmpty(folderHashes)) {
        return callback(null, listEmail);
    }
    ...
    FolderAPI.getFolderMembers(ctx, element.id, null, null, function(err, memberList) {
        ...
        _doesUserHaveRoleManager(user.id, element.groupId, function(err, hasRole) {
            if (err) {
                return callback(err);
            } 
            ...
            _removeDeletedUserToArray(memberList, user.id, function(err, newMemberList) {
                ...
                if (hasRole) {
                    ...
                    if (_.isEmpty(newMemberList)) { 
                            ...
                            AuthzAPI.updateRoles(element.groupId, update, function(err) {
                                ...
                                AuthzAPI.updateRoles(element.id, update, function(err) {
                                    ...
                                    FolderAPI.removeFolderFromLibrary(ctx, user.id, element.id, function(err) {
                                        ...
                                        return _transferRightToFolderLoop(folderHashes, ctx, user, archiveUser, listEmail, callback);
                                    });
                                });
                            });  
                        });  
                        ...
                        newMemberList.forEach(function(member, idx, array) {

                            if (member.role === AuthzConstants.role.MANAGER) {
 ...
 * @api private
 */
var _transferRightToDiscussions = function(ctx, user, archiveUser, listEmail, callback) {
    ...
    DiscussionAPI.getDiscussionsLibrary(ctx, user.id, null, null, function(err, discussions) {
        ...
        // Recursive loop
        _transferRightToDiscussionsLoop(discussions, ctx, user, archiveUser, listEmail, function(err, listEmail) {
            if (err) {
                return callback(err);
            }
            return callback(null, listEmail);
...
var _transferRightToDiscussionsLoop = function(discussionHashes, ctx, user, archiveUser, listEmail, callback) {

    discussionHashes = discussionHashes || [];

    // Return when there are no more folder in the array
    if (_.isEmpty(discussionHashes)) {
        return callback(null, listEmail);
    }
                        ...
                        _addToArchive(archiveUser.archiveId, user.id, element.id, function(err) {
                            ...
                            DiscussionAPI.setDiscussionPermissions(ctx, element.id, update, function(err) {
                                ...
                                DiscussionAPI.removeDiscussionFromLibrary(ctx, user.id, element.id, function(err) {
                                    ...
                                    // Recursive step inside the callback
                                    return _transferRightToDiscussionsLoop(discussionHashes, ctx, user, archiveUser, listEmail, callback); 
                                });
                            }); 
                        }); 
                    } else {
 ...
 * @api private
 */
var _transferRightToMeetings = function(ctx, user, archiveUser, listEmail, callback) {
    ...
    MeetingAPI.getMeetingsLibrary(ctx, user.id, null, null, function(err, meetings) {
        ...
        // Recursive loop
        _transferRightToMeetingsLoop(meetings, ctx, user, archiveUser, listEmail, function(err, listEmail) {
            if (err) {
                return callback(err);
            }
            return callback(null, listEmail);
...
var _transferRightToMeetingsLoop = function(meetingHashes, ctx, user, archiveUser, listEmail, callback) {

    meetingHashes = meetingHashes || [];

    // Return when there are no more folder in the array
    if (_.isEmpty(meetingHashes)) {
        return callback(null, listEmail);
    }
                    ...
                    if (_.isEmpty(newMemberList)) { 
                        ...
                        _setMeetingMembersNoVerif (ctx, element, update, function(err) {
                            ...
                            MeetingAPI.removeMeetingFromLibrary(ctx, user.id, element.id, function(err) {
                                ...
                                // Recursive step inside the callback
                                return _transferRightToMeetingsLoop(meetingHashes, ctx, user, archiveUser, listEmail, callback);
                            });
                        });
                    } else {
 ...
 * @api private
 */
var _transferRightToGroup = function(ctx, user, archiveUser, listEmail, callback) {
    ...
    GroupAPI.getMembershipsLibrary(ctx, user.id, null, null, function(err, groups) {
        ...
        // Recursive loop
        _transferRightToGroupLoop(groups, ctx, user, archiveUser, listEmail, function(err, listEmail) {
            if (err) {
                return callback(err);
            }
            return callback(null, listEmail);
...
var _transferRightToGroupLoop = function(groupHashes, ctx, user, archiveUser, listEmail, callback) {

    groupHashes = groupHashes || [];

    // Return when there are no more folder in the array
    if (_.isEmpty(groupHashes)) {
        return callback(null, listEmail);
    }
                            ...
                            AuthzAPI.updateRoles(element.id, update, function(err) {
                                ...
                                _setGroupMembersNoVerif (ctx, element, update, function(err) {
                                    ...
                                    AuthzAPI.updateRoles(element.id, update, function(err) {
                                        ...
                                        // Recursive step inside the callback
                                        return _transferRightToGroupLoop(groupHashes, ctx, user, archiveUser, listEmail, callback);
                                    });
                                });
                            });
                        });  
 ...
 * @api private
 */
var _removeDeletedUserToArray = function(memberList, userId, callback) {
    
    var newMemberList = [];

    memberList.forEach(function(element, i, arr) {
...
var _addToArchive = function(userArchiveId, principalId, elementId, callback) {
    ...
    if (!userArchiveId || !principalId || !elementId) {
        return callback(null);
    }

    // Add the element to data archive
    PrincipalsDAO.addDataToArchive(userArchiveId, principalId, [elementId], function(err) {
        if (err) {
            return callback(err);
        }
        return callback(null);  
...
/**
 * Check if the current user is manager of the resource 
 *
...
var _deleteResourceRights = function(ctx, user, archiveId, callback) {
    ...
    PrincipalsDAO.getDataFromArchive(archiveId, user.id, function(err, ids) {
        if (err) {
            return callback(err);
        } 
        if (_.isEmpty(ids)) {
                    ...
                    _ifFolderGetIdGroup(ctx, idResource, splitId[0], function(err, idFolder) {
                        ...
                        AuthzAPI.getAllAuthzMembers(idResource, function(err, memberIdRoles) {
                            ...
                            if (splitId[0] == "c") {
                                ...
                                _deleteRightToFolder(ctx, del, archiveId, id.resourceId, function(err) {
                                    if (err) {
                                        return callback(err);
                                    } else {
                                        finish ++;
