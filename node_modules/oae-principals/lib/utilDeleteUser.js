                        ...
                        ContentAPI.getContentMembersLibrary(ctx, element.id, null, null, function(err, memberList) {
                            ...
                            memberList.forEach(function(person, idx, array) { 
                                ...
                                if (idx === array.length - 1) { 
                                    _sendEmail(ctx, user, element, memberList, _.isEmpty(editors) && !hasManager, function(err) {
                                        if(err) {
                                            return callback(err);
                                        }
                                        console.log('mail ok')
    ...
    MeetingAPI.getMeetingsLibrary(ctx, user.id, null, null, function(err, meetings) {
        ...
        meetings.forEach(function(element, id, arr) {
            ...
            AuthzAPI.hasRole(user.id, element.id, AuthzConstants.role.MANAGER, function(err, hasRole) {
                ...
                // Delete all his right and remove from library
                MeetingAPI.removeMeetingFromLibrary(ctx, user.id, element.id, true, function(err) {
                    if(err) {
                        return callback(err);
                    }
                    if(hasRole) {
                        ...
                        MeetingAPI.getMeetingMembers(ctx, element.id, null, null, function(err, memberList) {
                            ...
                            if(_.isEmpty(memberList)) { 
                                ...
                                var update = {}; 
                                update[archiveUser.idArchive] = AuthzConstants.role.MANAGER;
                                MeetingAPI.setMeetingMembers(ctx, element.id, update, function(err) {
                                    if(err) {
                                        return callback(err);
                                    }
                                });
...
var _ifFolderGetIdGroup = function(ctx, idResource, idSplite, callback) {
    if(idSplite == "f") {
        ...
        return callback(null, idResource)
    }
};
...
var _deleteRightContent = function(ctx, del, idArchive, idResource, callback) {
    if(del) {
        ...
        ContentAPI.removeContentFromLibrary(ctx, idArchive, idResource, function() {
            if(err) {
                return callback(err);
            }
            console.log('content not removed')
            return callback();
        });
    }
};
...
var _deleteRightDiscussion = function(ctx, del, idArchive, idResource, callback) {
    if(del) {
        ...
        DiscussionsDAO.deleteDiscussion(ctx, idResource, function(err, discussionId) {
            if (err) {
                return callback(err);
            }
...
var _deleteRightFolder = function(ctx, del, idArchive, idResource, callback) {
    if(del) {
        // Remove folder
        FolderAPI.deleteFolder(ctx, idResource, true, function(err, content) {
            if(err) {
                return callback(err);
            }
...
var _deleteRightGroup = function(ctx, del, idArchive, idResource, callback) {
    if(del) {
        // Remove group
        PrincipalsDAO.deleteGroupFromPrincipals(idResource, function(err) {
            if(err) {
                return callback(err);
            }
        ...
        // TODO : find a solution to context
...
var _deleteRightMeeting = function(ctx, del, idArchive, idResource, callback) {
    if(del) {
        ...
        MeetingAPI.removeMeetingFromLibrary(ctx, idArchive, idResource, function() {
            if(err) {
                return callback(err);
            }
            console.log('meeting not removed')
            return callback();
        });
    }
};
