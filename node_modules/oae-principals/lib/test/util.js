...
var GroupTestUtil = require('oae-principals/lib/test/util');
var EmailAPI = require('oae-email');
var LibraryAPI = require('oae-library');
var OaeUtil = require('oae-util/lib/util');
var PrincipalsAPI = require('oae-principals');
var GroupAPI = require('oae-principals/lib/api.group');
var PrincipalsAPIUser = require('oae-principals/lib/api.user');
...
var FolderTestUtil = require('oae-folders/lib/test/util');
var MeetingAPI = require('oae-jitsi/lib/api.meetings');

var PrincipalsAPI = require('oae-principals');
var PrincipalsDelete = require('oae-principals/lib/delete');
var DefinitiveDeletionAPI = require('oae-principals/lib/definitiveDeletion');
 ...
 * @throws {AssertionError}                     Thrown if there is an issue deleting the groups or any of the assertions fail
 */
var assertDefinitiveDeletionUsersSucceeds = module.exports.assertDefinitiveDeletionUsersSucceeds = function(ctx, deletedUser, userArchive, callback) {
    
    // TODO : find another solution
    ctx.user = function () { return userArchive.user; };
    ctx.tenant = function () { return userArchive.user.tenant; };
    ctx.user().isAdmin = function () { return true; };
    ctx.tenant = function () { return userArchive.user.tenant; };
    userArchive.idArchive = userArchive.user.id;

    // Create manual user
    PrincipalsDAO.createUserArchive(userArchive.user.tenant.alias, userArchive.user.id, function(err, userArchiveCreated) {
        assert.ok(!err);
        assert.ok(userArchiveCreated);
        DefinitiveDeletionAPI.transferDataToArchive(ctx, deletedUser.user, userArchiveCreated, function(err) {
            assert.ok(!err);
            return callback(null, userArchiveCreated);
        }); 
    });
};
 ...
 */
var generateCollabdocs = module.exports.generateCollabdocs = function(restCtx, privacy, numToCreate, callback, _created) {
    _created = _created || [];
 ...
 * @throws {AssertionError}                     Thrown if the request failed
 */
var generateFiles = module.exports.generateFiles = function(restCtx, privacy, numToCreate, callback, _created) {
    _created = _created || [];
    if (_created.length === numToCreate) {
        return callback(null, _created);
    }
    RestAPI.Content.createFile(restCtx, 'name', 'description', privacy, _getPictureStream(), null, null, null, function(err, file) {
        assert.ok(!err);
        _created.push(file);
 ...
 * @throws {AssertionError}                     Thrown if the request failed
 */
var generateDiscussions = module.exports.generateDiscussions = function(restCtx, privacy, numToCreate, callback, _created) {
    _created = _created || [];
    if (_created.length === numToCreate) {
        return callback(null, _created);
    }
...
var generateMeetings = module.exports.generateMeetings = function(restCtx, manager, privacy, numToCreate, callback, _created) {
    ...
    RestAPI.MeetingsJitsi.createMeeting(restCtx, 'name', 'description', true, false, privacy, [], [], function (err, meeting) {
        assert.ok(!err);
 ...
 * @throws {AssertionError}                     Thrown if the request failed
 */
var generateLinks = module.exports.generateLinks = function(restCtx, privacy, numToCreate, callback, _created) {
    _created = _created || [];
    if (_created.length === numToCreate) {
        return callback(null, _created);
    }
 ...
 * @throws {AssertionError}                     Thrown if the request failed
 */
var generateGroups = module.exports.generateGroups = function(restCtx, privacy, numToCreate, callback, _created) {
    _created = _created || [];
    if (_created.length === numToCreate) {
        return callback(null, _created);
    }
    RestAPI.Group.createGroup(restCtx, 'Group title', 'Group description', 'public', 'yes', [], [], function(err, groupObj) {
        assert.ok(!err);
 ...
 * @throws {AssertionError}                     Thrown if the request failed
 */
var generateFolders = module.exports.generateFolders = function(user, privacy, numToCreate, callback, _created) {
    _created = _created || [];
    if (_created.length === numToCreate) {
        return callback(null, _created);
    }

    FolderTestUtil.assertCreateFolderSucceeds(user.restContext, 'name', 'description', privacy, [user], [], function(folder) {
 ...
 * @throws {AssertionError}                     Thrown if the request failed
 */
var generateRightGroup = module.exports.generateRightGroup = function(owner, contributor, right, group, callback) {
 ...
 * @throws {AssertionError}                     Thrown if the request failed
 */
var generateRightFolder = module.exports.generateRightFolder = function(owner, contributor, right, folder, callback) {
    var memberUpdates = {};
    memberUpdates[contributor.user.id] = right;
    FolderTestUtil.assertUpdateFolderMembersSucceeds(owner.restContext, owner.restContext, folder.id, memberUpdates, function(err) {
        assert.ok(!err);
...
var generateRightGroup = module.exports.generateRightGroup = function(owner, contributor, right, group, callback) {
    ...
    // TODO ? 
    owner.restContext.tenant = function () { return owner.user.tenant };
    owner.restContext.user = function () { return owner.user };
