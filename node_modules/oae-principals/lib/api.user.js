...
var Validator = require('oae-util/lib/validator').Validator;
var UtilDeleteUser = require('oae-principals/lib/definitiveDeletion');
...
var deleteUser = module.exports.deleteUser = function(ctx, userId, callback) {
    ...
    canDeleteUser(ctx, userId, function(err, canDelete, user) {
        ...
        PrincipalsDAO.getPrincipal(userId, function(err, user) {
            ...
            UtilDeleteUser.getOrCreateArchiveUser(ctx, user, function(err, archiveUser) {
                if(err) {
                    return callback(err);
                }

                UtilDeleteUser.transferDataToArchive(ctx, user, archiveUser, function(err) {
                    if (err) {
                        return callback(err);
                    }
                    ...
                    PrincipalsDAO.deletePrincipal(userId, function(err) {
                        ...
                        UtilDeleteUser.getConfig(ctx, "oae-principals/user/delete", 2, function (err, month) {
                            if(err) {
                                return callback(err);
                            }

                            UtilDeleteUser.getConfig(ctx, "oae-tenants/timezone/timezone", "Etc/UTC", function (err, timezone) {
                                if(err) {
                                    return callback(err);
                                }
                                ...
                                newMonth = m + month;
                                s = today.getSeconds();
                                taskDate.setSeconds(s+60);
                                //taskDate.setMonth(newMonth);

                                // Context of cron
                                var context = {ctx: ctx, user: user, alias: user.tenant.alias};
                                ...
                                var task = new CronJob(taskDate, function() {
                                    ...
                                    PrincipalsDAO.restorePrincipal(user.id, function(err) {
                                        ...
                                        // Delete user 
                                        UtilDeleteUser.definitiveDeletion(context.ctx, context.user, context.alias, function(err, finish) {
                                            if(err) {
                                                console.log(err);
                                            }
                                            if(finish === true) {
