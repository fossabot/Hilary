...
var deleteUser = module.exports.deleteUser = function(ctx, userId, callback) {
    ...
    canDeleteUser(ctx, userId, function(err, canDelete, user) {
        ...
        PrincipalsDAO.getPrincipal(userId, function(err, user) {
            ...
            UserDeletionUtil.getOrCreateArchiveUser(ctx, user, function(err, archiveUser) {
                if (err) {
                    return callback(err);
                }

                UserDeletionUtil.transferDataToArchive(ctx, user, archiveUser, function(err) {
                    if (err) {
                        return callback(err);
                    }
                    ...
                    PrincipalsDAO.deletePrincipal(userId, function(err) {
                        ...
                        // Grab the configuration field. This will return the number of months after which an archived user is deleted
                        var month = PrincipalsConfig.getValue(ctx.tenant().alias, 'user', 'delete');

                        // Grab the configuration field. This will return the timezone
                        var timezone = PrincipalsConfig.getValue(ctx.tenant().alias, 'timezone', 'timezone');
                        ...
                        var task = new CronJob(taskDate, function() {
                            ...
                            PrincipalsDAO.restorePrincipal(user.id, function(err) {
                                ...
                                // Delete user 
                                UserDeletionUtil.definitiveDeletion(context.ctx, context.user, context.alias, function(err, finish) {
                                    if (err) {
                                        console.log(err);
                                    }
                                    if (finish === true) {
