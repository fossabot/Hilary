            ...
            _deleteFollowers(user, function(err) {
                ...
                _deleteRightGroup(ctx, user, function(err) {
                    ...
                    PrincipalsDAO.deletePrincipal2(ctx, user, function(err) {
                        ...
                        // Notify consumers that a user has been deleted
                        log().info({'userId': user.id, 'name': 'oae-principals'}, 'Deleted user with a mapped login id');
                        ...
                        // TODO resolve error : An uncaught exception was raised to the application
                        //                      TypeError: boolean is not a function
                        //                      Hilary/node_modules/oae-discussions/lib/api.discussions.js:222:36
                    ...
                    FolderAPI.getFolderMembers(ctx, element.id, null, null, function(err, memberList) {
                        ...
                        if(memberList.length === 1) {
                            ...
                            if(user.id === memberList[0].profile.id) {
                                FolderAPI.deleteFolder(ctx, element.id, false, function(err) {
                                    if(err) {
                                        return callback(err);
                                    }
                                    return callback(); 
            ...
            AuthzAPI.hasRole(user.id, element.id, AuthzConstants.role.MANAGER, function(err, hasRole) {
                ...
                if(hasRole) {
                    ...
                    DiscussionAPI.getDiscussionMembers(ctx, element.id, null, null, function(err, memberList) {
                        ...
                        // If there is no manager/member/editor delete it, else do nothing 
                        if(memberList.length === 1) {
                            DiscussionAPI.deleteDiscussion(ctx, element.id, false, function(err) {
                                if(err) {
                                    return callback(err);
                                }
                                return callback(); 
