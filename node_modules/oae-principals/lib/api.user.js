 ...
 */
var deleteUser = module.exports.deleteUser = function(ctx, userId, callback) {
    var validator = new Validator();
    validator.check(userId, {'code': 400, 'msg': 'A valid user id must be provided'}).isUserId();
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    ...
    canDeleteUser(ctx, userId, function(err, canDelete, user) {
        if (err) {
            return callback(err);
        } else if (!canDelete) {
        ...
        PrincipalsDAO.getPrincipal(userId, function(err, user) {
            ...
            PrincipalsDAO.deletePrincipal(userId, function(err) {
                if (err) {
                    return callback(err);
                }

                // Notify consumers that a user has been deleted
                PrincipalsEmitter.emit(PrincipalsConstants.events.DELETED_USER, ctx, user, callback);

                _getConfig(ctx, "oae-principals/user/delete", 2, function (err, month) {
                    if(err) {
                        return callback(err);
                    }
                    _getConfig(ctx, "oae-tenants/timezone/timezone", "Etc/UTC", function (err, timezone) {
                        if(err) {
                            return callback(err);
                        }
                        ...
                        // Modify the Date adding X months
                        m = today.getMonth();
                        newMonth = m + month;
                        s = today.getSeconds();
                        taskDate.setSeconds(s+5);
                        //taskDate.setMonth(newMonth);

                        // Context of cron
                        var context = {ctx: ctx, user: user};

                        // Create cron to definitely delete a user
                        var task = new CronJob(taskDate, function() {
                            // Restore user to avoid errors
                            PrincipalsDAO.restorePrincipal(user.id, function(err) {
                                if (err) {
                                    return callback(err);
                                }
                                // Delete user 
                                _definitiveDeletion(context.ctx, context.user, function(err, finish) {
                                    if(err) {
                                        console.log(err);
                                    }
                                    if(finish === true) {
 ...
 * @param  {Object}     callback.err    An error that occured, if any
 */
var _getConfig = function(ctx, configkey, defaultValue, callback) {
    if (_.isEmpty(ctx)) {
 ...
 * @param  {Object}     callback.err    An error that occured, if any
 */
 // TODO public function for unit-test 
var _definitiveDeletion = module.exports._definitiveDeletion = function(ctx, user, callback) {
    callback = callback || function() {};

    // Delete rights and his content if there is no manager/member/editor
    _deleteRightContent(ctx, user, function(err) {
        if (err) {
            return callback(err);
        }

        // Delete rights and his folder if there is no manager/member/editor
        _deleteRightFolder(ctx, user, function(err) {
            if (err) {
                return callback(err);
            } 

            // Delete rights and his discussions if there is no manager/view
            _deleteRightDiscussions(ctx, user, function(err) {
                if (err) {
                    return callback(err);
                }  
                ...
                _deleteFollowers(user, function(err) {
                    ...
                    // Delete groups
                    _deleteRightGroup(ctx, user, function(err) {
                        if (err) {
                            return callback(err);
                        }
...
/**
 * Remove all his rights on contents
 *
 * @param  {Context}    ctx             Standard context object containing the current user and the current tenant
 ...
 * @param  {Object}     callback.err    An error that occured, if any
 */
var _deleteRightContent = function(ctx, user, callback) {
    // Get all documents 
        ...
        contents.forEach(function(element, id, arr) {
            ...
            AuthzAPI.hasRole(user.id, element.id, AuthzConstants.role.MANAGER, function(err, hasRole) {
                ...
                ContentAPI.removeContentFromLibrary(ctx, user.id, element.id, true, function(err) {
                    ...
                    // If he's not a manager, do nothing, else...
                    if(hasRole) {
                        ...
                        ContentAPI.getContentMembersLibrary(ctx, element.id, null, null, function(err, memberList) {
                            ...
                            // If member list is empty - nobody has any rights in this resource - delete it
                            if(_.isEmpty(memberList)) {
                                ContentAPI.deleteContent(ctx, element.id, function(err) {
                                    if(err) {
                                        return callback(err);
                                    }
                                    return callback(); 
                            ...
                            memberList.forEach(function(person, idx, array) {
                                ...
                                // If we have already find a manager/editor, do nothing
                                if(stop) { return; }
                                // Look at his role
                                if(person.role === AuthzConstants.role.MANAGER || person.role === AuthzConstants.role.EDITOR) {
                                    // TODO what to do if there is no more manager in the resource ? 
                                    stop = true;
                                    return;
                                }  

                                // If it's the last iteration and we don't find any manager/editor, delete the resource
...
/**
 * Remove all his rights on folders
 *
 * @param  {Context}    ctx             Standard context object containing the current user and the current tenant
 * @param  {String}     user            The user to delete
 * @param  {Function}   callback        Standard callback function
 * @param  {Object}     callback.err    An error that occured, if any
 */
var _deleteRightFolder = function(ctx, user, callback) {
    // Get all documents 
                ...
                if(hasRole){
                    ...
                    FolderAPI.getFolderMembers(ctx, element.id, null, null, function(err, memberList) {
                        ...
                        if(memberList.length === 1) {
                            // If the member is the user removed, delete the folder
                            if(user.id === memberList[0].profile.id) {
                                FolderAPI.deleteFolder(ctx, element.id, function(err) {
                                    if(err) {
                                        return callback(err);
                                    }
                                    return; 
...
/**
 * Remove all his rights on discussions
 *
 * @param  {Context}    ctx             Standard context object containing the current user and the current tenant
 * @param  {String}     user            The user to delete
 * @param  {Function}   callback        Standard callback function
 * @param  {Object}     callback.err    An error that occured, if any
 */
var _deleteRightDiscussions = function(ctx, user, callback) {
    // Get all documents 
            ...
            AuthzAPI.hasRole(user.id, element.id, AuthzConstants.role.MANAGER, function(err, hasRole) {
                ...
                if(hasRole) {
                    ...
                    DiscussionAPI.getDiscussionMembers(ctx, element.id, null, null, function(err, memberList) {
                        ...
                        // If there is no manager/member/editor delete it, else do nothing 
                        if(memberList.length === 1) {
                            DiscussionAPI.deleteDiscussion(ctx, element.id, function(err) {
                                if(err) {
                                    return callback(err);
                                }
                                return; 
                            ...
                            memberList.forEach(function(person, idx, array) {
                                ...
                                if(stop) { return; } 
                                // If the user is the person who will be deleted, do nothing
                                if(person.id === user.id){ return; }

                                // Look at his role
                                if(person.profile.role === AuthzConstants.role.MANAGER) {
                                    // TODO what to do if there is no more manager in the resource ? 
                                    stop = true;
                                    return;
                                }  

                                // If it's the last iteration and we don't find any manager, delete the resource
                                if (idx === array.length - 1) { 
                                    DiscussionAPI.deleteDiscussion(ctx, element.id, function(err) {
                                        if(err) {
                                            return callback(err);
 ...
 * @param  {Object}     callback.err    An error that occured, if any
 */
var _deleteRightGroup = function(ctx, user, callback) {
    // Get all documents 
            ...
            AuthzAPI.hasRole(user.id, element.id, AuthzConstants.role.MANAGER, function(err, hasRole) {
                ...
                if(hasRole) {
                    ...
                    GroupAPI.getMembersLibrary(ctx, element.id, null, null, function(err, memberList) {
                        ...
                        // If there is no manager/member/editor delete it, else do nothing 
                        if(memberList.length === 1) {
                            GroupAPI.deleteGroup(ctx, element.id, function(err) {
                                if(err) {
                                    return callback(err);
                                }
                                return callback(); 
                            ...
                            // Delete groupe from the table Principals
 ...
 */
var deleteOrRestoreUsersByTenancy = module.exports.deleteOrRestoreUsersByTenancy = function (ctx, tenantAlias, disableUsers, callback) {
    getAllUsersForTenant(ctx, tenantAlias, function(err, users) {
        if (err) {
            callback(err);
        }
        ...
        if (disableUsers) {
            ...
            _restorePrincipals(users, function (err, users) {
                 if (err) {
                    callback(err);
                }
                callback(null, users);
 ...
 * @api private
 */
var getAllUsersForTenant = module.exports.getAllUsersForTenant = function (ctx, tenantAlias, callback) {
    tenantAlias = tenantAlias || ctx.user().tenant.alias;
    var tenant = TenantsAPI.getTenant(tenantAlias);
    ...
    PrincipalsDAO.getAllUsersForTenant(tenantAlias, function(err, users) {
        if (err) {
            return callback(err);
        }
 ...
 * @api private
 */
var _deletePrincipals = function (usersToDelete, afterDeleted) {
    async
        .mapSeries(usersToDelete, function(eachUser, transformed) {
            PrincipalsDAO.deletePrincipal(eachUser.id, function(err) {
 ...
 * @api private
 */
var _restorePrincipals = function (usersToRestore, afterRestored) {
    async
        .map(usersToRestore, function(eachUser, transformed) {
            PrincipalsDAO.restorePrincipal(eachUser.id, function(err) {
                transformed(null, eachUser);
            });
 ...
 * @param  {Object}     callback.err    An error that occured, if any
 */
var restoreUser = module.exports.restoreUser = function(ctx, userId, callback) {
    var validator = new Validator();
    validator.check(userId, {'code': 400, 'msg': 'A valid user id must be provided'}).isUserId();
    if (validator.hasErrors()) {
        return callback(validator.getFirstError());
    ...
    canRestoreUser(ctx, userId, function(err, canRestore, user) {
        if (err) {
            return callback(err);
        } else if (!canRestore) {
        ...
        PrincipalsDAO.restorePrincipal(userId, function(err) {
            if (err) {
                return callback(err);
            }
 ...
 */
var canDeleteUser = module.exports.canDeleteUser = function(ctx, userId, callback) {
    if (!ctx.user()) {
    ...
    PrincipalsDAO.getPrincipal(userId, function(err, user) {
        if (err) {
            return callback(err);
        } else if (ctx.user().id !== userId && !ctx.user().isAdmin(user.tenant.alias)) {
 ...
 * @param  {User}       callback.user       The user that was fetched to perform the checks. Will not be specified if the authorization fails
 */
var canRestoreUser = module.exports.canRestoreUser = function(ctx, userId, callback) {
    if (!ctx.user()) {
        return callback(null, false);
    }
...
var _sendEmailToken = function(ctx, user, email, token, callback) {
    ...
    PrincipalsDAO.storeEmailToken(user.id, email, token, function(err) {
        ...
        var verificationUrl = TenantsUtil.getBaseUrl(tenant) + '/?verifyEmail=' + encodeURIComponent(token);
...
var verifyEmail = module.exports.verifyEmail = function(ctx, userId, token, callback) {
    ...
    PrincipalsDAO.getPrincipal(userId, function(err, user) {
        ...
        PrincipalsDAO.getEmailToken(userId, function(err, email, persistedToken) {
            ...
            PrincipalsUtil.verifyEmailAddress(ctx, user, email, function(err, updatedUser) {
                if (err) {
                    return callback(err);
                }
