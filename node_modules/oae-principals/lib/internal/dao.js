...
const Group = require('oae-principals/lib/model').Group;
...
const UserArchive = require('oae-principals/lib/model').UserArchive;
...
var createUser = module.exports.createUser = function(user, callback) {
    ...
    var values = {
        ...
        'isUserArchivedd': user.isUserArchivedd
    };
...
/**
 * Permanently delete a user and his old resources
 *
 * @param  {String}     principalId     The id of the principal to permanently delete
 * @param  {Function}   callback        Standard callback function
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var fullyDeletePrincipal = module.exports.fullyDeletePrincipal = function(ctx, user, callback) {
    var deleted = Date.now();
    ...
    AuthzDelete.isDeleted([user.id], function(err, wasDeleted) {
        ...
        // AuthenticationAPI.getUserLoginIds(ctx, user.id, function(err, login) { // TODO(claire) : use this line 
            if (err) {
                return callback(err);
            }
                    ...
                    Cassandra.runQuery('DELETE FROM "PrincipalsByEmail" where "email" = ? AND "principalId" = ?', [user.email, user.id], function(err, rows) {
                        ...
                        Cassandra.runQuery('DELETE FROM "AuthenticationUserLoginId" WHERE "userId" = ?', [user.id], function(err, rows) {
                            ...
                            if (loginId && loginId.tenantAlias && loginId.provider && loginId.externalId) {
                                Cassandra.runQuery('DELETE FROM "AuthenticationLoginId" WHERE "loginId" = ?', [ loginId.tenantAlias + ':' + loginId.provider + ':' + loginId.externalId], function(err, rows) {
                                    if (err) {
                                        return callback(err);
                                    }
...
var _hashToUser = function(hash) {
    var user = new User(hash.tenantAlias, hash.principalId, hash.displayName, hash.email, {
        ...
        'isUserArchived': hash.isUserArchived
    });
    return user;
};
...
var getAllUsersForTenant = module.exports.getAllUsersForTenant = function(tenantAlias, callback) {
    ...
    Cassandra.runAutoPagedQuery(query, [tenantAlias], function(err, rows) {
        if (err) {
            return callback(err);
        } else if (_.isEmpty(rows)) {
            return callback(null, []);
 ...
 */
var createArchivedUser = module.exports.createArchivedUser = function(alias, createdUserId, callback) {

    // Prepare query
    var query = 'INSERT INTO "ArchiveByTenant" ("tenantAlias", "archiveId") VALUES (?, ?)';
 ...
 */
var getDataFromArchive = module.exports.getDataFromArchive = function(archiveId, principalId, callback) {

    // Verify if an archive user exist
    Cassandra.runQuery('SELECT * FROM "DataArchive" WHERE "archiveId" = ? AND "principalId" = ?', [archiveId, principalId], function(err, rows) {
        if (err) {
            return callback(err);
        } else if (_.isEmpty(rows)) {
            return callback(null, []);
        ...
        var userArchive = _.map(rows, Cassandra.rowToHash);
 ...
 * @param  [User]           callback.users      The list of users for the given tenancy
 */
var addDataToArchive = module.exports.addDataToArchive = function(archiveId, principalId, resourceId, callback) {

    var stringResource = resourceId.join();

    // Prepare query
    var query = 'INSERT INTO "DataArchive" ("archiveId", "principalId", "resourceId") VALUES (?, ?, ?)';
    var parameters = [archiveId, principalId, stringResource];
 ...
 * @param  [User]           callback.users      The list of users for the given tenancy
 */
var removeDataInArchive = module.exports.removeDataInArchive = function(archiveId, principalId, callback) {

    Cassandra.runQuery('DELETE FROM "DataArchive" WHERE "archiveId" = ? AND "principalId" = ?', [archiveId, principalId], function(err, rows) {
        if (err) {
            return callback(err);
        }
        return callback();
 ...
 */
var getArchivedUser = module.exports.getArchivedUser = function(alias, callback) {
