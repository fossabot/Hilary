...
var fullyDeletePrincipal = module.exports.fullyDeletePrincipal = function(ctx, user, callback) {
    ...
    AuthzDelete.isDeleted([user.id], function(err, wasDeleted) {
        ...
        require('oae-authentication').getUserLoginIdsDAO(user.id, function(err, login) {
            ...
            AuthzDelete.setDeleted(user.id, function(err) {
                ...
                Cassandra.runQuery('DELETE FROM "Principals" where "principalId" = ?', [user.id], function(err, rows) {
                    ...
                    Cassandra.runQuery('DELETE FROM "PrincipalsByEmail" where "email" = ? AND "principalId" = ?', [user.email, user.id], function(err, rows) {
                        ...
                        var loginId = new LoginId(user.tenant.alias, AuthenticationConstants.providers.LOCAL, login.local.externalId);
...
var addDataToArchive = module.exports.addDataToArchive = function(archiveId, principalId, resourceId, callback) {
    ...
    var queries = [];
    var stringResource = "";
    stringResource = resourceId.join();
