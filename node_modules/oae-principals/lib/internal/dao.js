...
var UserArchive = require('oae-principals/lib/model').UserArchive;
...
let RESTRICTED_FIELDS = ['acceptedTC', 'admin:tenant', 'admin:global', 'deleted'];
...
var createGroup = module.exports.createGroup = function(groupId, tenantAlias, displayName, description, visibility, joinable, createdBy, callback) {
    ...
    var created = new Date(lastModified);
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var deletePrincipal2 = module.exports.deletePrincipal2 = function(ctx, user, callback) {
    var deleted = Date.now();
    ...
    AuthzDelete.isDeleted([user.id], function(err, wasDeleted) {
        ...
        require('oae-authentication').getUserLoginIds(ctx, user.id, function(err, login){
            ...
            AuthzDelete.setDeleted(user.id, function(err) {
                ...
                Cassandra.runQuery('DELETE FROM "Principals" where "principalId" = ?', [user.id], function(err, rows) {
                    if (err) {
                        return callback(err);
                    }

                    // Delete user from table PrincipalsByEmail
                    Cassandra.runQuery('DELETE FROM "PrincipalsByEmail" where "email" = ? AND "principalId" = ?', [user.email, user.id], function(err, rows) {
                        if (err) {
                            return callback(err);
                        }
                    
                        var loginId = new LoginId(user.tenant.alias, AuthenticationConstants.providers.LOCAL, login.local);

                        // Delete user from table AuthenticationUserLoginId
                        Cassandra.runQuery('DELETE FROM "AuthenticationUserLoginId" WHERE "userId" = ?', [user.id], function(err, rows) {
                            if (err) {
                                return callback(err);
                            }

                            // Delete user from table AuthenticationLoginId
                            if(loginId.externalId !== undefined){
                                Cassandra.runQuery('DELETE FROM "AuthenticationLoginId" WHERE "loginId" = ?', [_flattenLoginId(loginId)], function(err, rows) {
                                    if (err) {
                                        return callback(err);
                                    }

                                    // Update the cache,
                                    return OaeUtil.invokeIfNecessary(isUser(user.id), _updateCachedUser, user.id, {'deleted': deleted}, callback);
                                });
                            }
                        });
                    });
 ...
 * @param  {Object}     callback.err    An error that occurred, if any
 */
var deleteGroupFromPrincipals = module.exports.deleteGroupFromPrincipals = function(group, callback){
    // Delete user from table Principals
    Cassandra.runQuery('DELETE FROM "Principals" where "principalId" = ?', [group.id], function(err, rows) {
        if (err) {
            return callback(err);
        }
    });
...
/**
 * Imported from Authentification api
...
var getEmailToken = module.exports.getEmailToken = function(userId, callback) {
    Cassandra.runQuery('SELECT * FROM "PrincipalsEmailToken" WHERE "principalId" = ?', [userId], function(err, rows) {
        ...
        var token = _.first(rows).get('token');
        var email = _.first(rows).get('email');
 ...
 * @api private
 */
var getPrincipalFromCassandra = module.exports.getPrincipalFromCassandra = function(principalId, callback) {
    if (isUser(principalId)) {
        // The user wasn't cached, fetch from the DB
        return _getPrincipalFromCassandra(principalId, callback);
    } 
 ...
 * @api private
 */
var getAllUsersForTenant = module.exports.getAllUsersForTenant = function(tenantAlias, callback) {
    Cassandra.runQuery('SELECT * FROM "Principals" WHERE "tenantAlias" = ?', [tenantAlias], function(err, rows) {
        if (err) {
            return callback(err);
        } else if (_.isEmpty(rows)) {
            return callback(null, []);
 ...
 */
var createUserArchive = module.exports.createUserArchive = function(alias, createdUserId, callback) {

    // Prepare query
    var query = 'INSERT INTO "ArchiveByTenant" ("aliasTenant", "idArchive") VALUES (?, ?)';
    var parameters = [alias, createdUserId];
    ...
    Cassandra.runQuery(query, parameters, function (err) {
        ...
        var userArchive = new UserArchive(alias, createdUserId);
 ...
 *
 * @param  {String}         idArchive           The archive id ot the principal tenant 
 ...
 */
var getDataFromArchive = module.exports.getDataFromArchive = function(idArchive, principalId, callback) {

    // Verify if an archive user exist
    Cassandra.runQuery('SELECT * FROM "DataArchive" WHERE "idArchive" = ? AND "principalId" = ?', [idArchive, principalId], function(err, rows) {
        if (err) {
            return callback(err);
        } 
        userArchive = _.chain(rows).map(_rowToArchive);
        return callback(null, userArchive._wrapped);
    });
};
 ...
 * @param  {String}         idArchive           The archive id ot the principal tenant 
 * @param  {String}         principalId         The principal id of the removed user
 * @param  {String}         idResources         The resource id of the removed user
 * @param  {Function}       callback            Standard callback function
 * @param  {Object}         callback.err        An error that occurred, if any
 * @param  [User]           callback.users      The list of users for the given tenancy
 */
var addDataInArchive = module.exports.addDataInArchive = function(idArchive, principalId, idResources, callback) {

    var queries = [];

    // Foreach resources, add queries to array
    idResources.forEach(function(element, id, arr) {
        queries.push({'query': 'INSERT INTO "DataArchive" ("idArchive", "principalId", "idResources") VALUES (?, ?, ?)', 'parameters': [idArchive, principalId, element]});  
...
/**
 * Remove datas from user archive
 *
 * @param  {String}         idArchive           The archive id ot the principal tenant 
 * @param  {String}         principalId         The principal id of the removed user
 * @param  {Function}       callback            Standard callback function
 ...
 * @param  [User]           callback.users      The list of users for the given tenancy
 */
var removeDataInArchive = module.exports.removeDataInArchive = function(idArchive, principalId, callback) {

    Cassandra.runQuery('DELETE FROM "DataArchive" WHERE "idArchive" = ? AND "principalId" = ?', [idArchive, principalId], function(err, rows) {
        if (err) {
            return callback(err);
        }
        return callback(null);
 ...
 * @param  {Function}       callback                Standard callback function
 * @param  {Object}         callback.err            An error that occurred, if any
 * @param  [UserArchive]    callback.userArchive    The archive id ot the tenant
 */
var getUserArchive = module.exports.getUserArchive = function(alias, callback) {

    Cassandra.runQuery('SELECT * FROM "ArchiveByTenant" WHERE "aliasTenant" = ?', [alias], function(err, rows) {
        if (err) {
            return callback(err);
        } 
        userArchive = _.chain(rows).map(_rowToArchive);
        return callback(null, userArchive._wrapped[0]);
    });  
}; 

/**
 * Return a user archive
 * @api private
 */
var _rowToArchive = function(row) {
