...
var getAllUsersForTenant = module.exports.getAllUsersForTenant = function(tenantAlias, callback) {
    Cassandra.runQuery('SELECT * FROM "Principals" WHERE "tenantAlias" = ?', [tenantAlias], function(err, rows) {
        ...
        var users = _.map(rows, function(row) {
            if (isUser(row.get('principalId').value)) {
...
/**
 * Get a user archive or create it
 *
 ...
 * @api private
 */
var getOrCreateArchiveUser = module.exports.getOrCreateArchiveUser = function(ctx, tenant, callback) {
    ...
    Cassandra.runQuery('SELECT * FROM "ArchiveByTenant" WHERE "aliasTenant" = ?', [tenant.alias], function(err, rows) {
        if (err) {
            return callback(err);
        } else if (_.isEmpty(rows)) {

            // TODO : maybe change "u" for "a" ?
            var id = AuthzUtil.toId('u', tenant.alias, ShortId.generate());
            var resourceId = AuthzUtil.getResourceFromId(id).resourceId;

            // Persist the user object
            var user = {
                'id': id,
                'publicAlias': tenant.alias + ' archive',
                'tenant': {'alias': tenant.alias},
                'displayName': tenant.alias + ' archive',
                'visibility': 'private',
                'email': tenant.alias + 'Archive@' + tenant.emailDomains[0],
                'emailPreference': 'immediate',
                'locale': ctx.locale(),
                'picture': {'smallUri': null, 'mediumUri': null, 'largeUri': null},
                'acceptedTC': null
            };

            // Prepare query
            var query = 'INSERT INTO "ArchiveByTenant" ("aliasTenant", "idArchive") VALUES (?, ?)';
            var parameters = [tenant.alias, id];
            ...
            Cassandra.runQuery(query, parameters, function (err) {
                ...
                // Create user
                createUser(user, function(err, userArchive) {
                    if(err){
                        return callback(err);
                    }
...
var getUserArchive = module.exports.getUserArchive = function(alias, principalId, callback) {
    // Verify if an archive user exist
