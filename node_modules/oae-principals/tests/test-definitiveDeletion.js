                    ...
                    PrincipalsDAO.getPrincipal(userToDelete.user.id, function (err, retrievedUser) {
                        ...
                        PrincipalsTestUtil.assertDefinitiveDeletionUsersSucceeds(camAdminRestContext, userToDelete, userArchive, function(err, resUserArchive) { 
                            ...
                            PrincipalsDefinitiveDeletionAPI.definitiveDeletion(camAdminRestContext, userToDelete.user, resUserArchive.tenantAlias, function(err) {
                                ...
                                // TODO get files path
                                fs.readFile(split.slice(0, split.length - 1).join("/") + "/files/" + pathSmallPicture[1], function (err, data) {
                                    assert.ok(err);
                                    assert.ok(!data);
                                    fs.readFile(split.slice(0, split.length - 1).join("/") + "/files/" + pathMediumPicture[1], function (err, data) {
                                        assert.ok(err);
                                        assert.ok(!data);
                                        fs.readFile(split.slice(0, split.length - 1).join("/") + "/files/" + pathLargePicture[1], function (err, data) {
                                            assert.ok(err);
                                            assert.ok(!data);
                                            return callback()
                                        });
