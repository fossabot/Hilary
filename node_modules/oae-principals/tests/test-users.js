...
//             TenantsTestUtil.createTenantAndWait(globalAdminRestContext, recaptchaTenantAlias, recaptchaTenantAlias, host, null, function(err, recaptchaTenant) {
//                 assert.ok(!err);
...
//                         'visibility': 'public'
//                     };
//                     var recaptchaAnonymousRestContext = TestsUtil.createTenantRestContext(host);
...
//                             assert.equal(createdUser.email, params.email.toLowerCase());
//                             assert.equal(createdUser.profilePath, '/user/' + createdUser.tenant.alias + '/' + AuthzUtil.getResourceFromId(createdUser.id).resourceId);
//                             var userRestContext = TestsUtil.createTenantRestContext(global.oaeTests.tenants.cam.host, params.username, params.password);
...
//                                         PrincipalsTestUtil.assertCreateUserSucceeds(userRestContext, params, function(userObj) {
...
//          */
//         it('verify the email address has to belong to the configured tenant\'s email domain when creating a local user', function(callback) {
...
//             TenantsTestUtil.createTenantAndWait(globalAdminRestContext, tenantAlias, tenantAlias, tenantHost, {}, function(err) {
//                 assert.ok(!err);
...
//                     var emailDomain1 = tenantHost;
//                     var emailDomain2 = TenantsTestUtil.generateTestTenantHost();

//                     // Verify accounts can be created for any email domain
//                     _verifyCreateUserWithEmailDomain(tenantHost, emailDomain1, function() {
...
//                                 _verifyCreateUserWithEmailDomain(tenantHost, emailDomain1, function() {
//                                     _verifyCreateUserWithEmailDomainFails(tenantHost, emailDomain2, function() {
//                                         return callback();
//                                     });
//                                 });
//                             });
...
//          */
//         it('verify administrators can create user accounts with an email address that does not match the configured email domain', function(callback) {
//             // Create a tenant but don't configure an email domain yet
//             var tenantAlias = TenantsTestUtil.generateTestTenantAlias();
//             var tenantHost = TenantsTestUtil.generateTestTenantHost();
...
//                 };
//                 PrincipalsTestUtil.assertCreateUserSucceeds(tenantAdminRestContext, params, function() {
...
//          * @api private
//          */
//         var _verifyCreateUserWithEmailDomain = function(tenantHost, emailDomain, callback) {
...
//          * @api private
//          */
//         var _verifyCreateUserWithEmailDomainFails = function(tenantHost, emailDomain, callback) {
//             var restContext = TestsUtil.createTenantRestContext(tenantHost);
//             var params = {
//                 'username': TestsUtil.generateTestUserId(),
//                 'password': 'password',
...
//             var tenantAlias = TenantsTestUtil.generateTestTenantAlias();
//             var tenantHost = 'displayname.import.tests.com';
//             TestsUtil.createTenantWithAdmin(tenantAlias, tenantHost, function(err, tenant, tenantAdminRestContext) {
//                 assert.ok(!err);
...
//                         _verifyUser(err, nicolaasMeObj, 'Nicolaas Matthijs', 'Nicolaas Matthijs', 'nicolaas@displayname.import.tests.com');
...
//                                 _verifyUser(err, stuartMeObj, 'Stuart D. Freeman', 'Stuart D. Freeman', 'stuart@displayname.import.tests.com');
...
//                                     _verifyUser(err, stephenMeObj, 'Stephen Thomas', 'Stephen Thomas', 'stephen@displayname.import.tests.com');
...
//                                         RestAPI.User.getMe(nicolaasRestContext, function(err, nicolaasMeObj) {
//                                             _verifyUser(err, nicolaasMeObj, 'user-zfdHliPLa', 'Nicolaas Matthijs', 'nicolaas@displayname.import.tests.com');
...
//                                                         // Verify that the update is reflected in the me feed
//                                                         RestAPI.User.getMe(nicolaasRestContext, function(err, nicolaasMeObj) {
//                                                             _verifyUser(err, nicolaasMeObj, 'N. Matthijs', 'Nicolaas Matthijs', 'nicolaas@displayname.import.tests.com');
...
//             var tenantAlias = TenantsTestUtil.generateTestTenantAlias();
//             var tenantHost = 'alias.import.tests.com';
//             TestsUtil.createTenantWithAdmin(tenantAlias, tenantHost, function(err, tenant, tenantAdminRestContext) {
//                 assert.ok(!err);

//                 // Import users as a global admin using a local authentication strategy
//                 PrincipalsTestUtil.importUsers(globalAdminRestContext, tenantAlias, getDataFileStream('users-with-password-alias.csv'), 'local', null, function(err) {
//                     assert.ok(!err);

//                     var nicolaasRestContext = TestsUtil.createTenantRestContext(tenantHost, 'user-zfdHliPLa', 'password1');
//                     RestAPI.User.getMe(nicolaasRestContext, function(err, nicolaasMeObj) {
//                         _verifyUser(err, nicolaasMeObj, 'Nicolaas Matthijs', 'Nicolaas Matthijs', 'nicolaas@alias.import.tests.com');
...
//                                 // provided in the CSV file. This time, the import is attempted as a tenant admin
//                                 PrincipalsTestUtil.importUsers(tenantAdminRestContext, tenantAlias, getDataFileStream('users-with-password-alias.csv'), 'local', null, function(err) {
//                                     assert.ok(!err);
...
//                                             // Verify that the update is reflected in the me feed
//                                             RestAPI.User.getMe(nicolaasRestContext, function(err, nicolaasMeObj) {
//                                                 _verifyUser(err, nicolaasMeObj, 'Nicolaas Matthijs', 'Nico', 'nicolaas@alias.import.tests.com');
...
//                                                         // the display name provided in the CSV file
//                                                         PrincipalsTestUtil.importUsers(tenantAdminRestContext, tenantAlias, getDataFileStream('users-with-password-alias.csv'), 'local', true, function(err) {
//                                                             assert.ok(!err);
...
//             var tenantAlias = TenantsTestUtil.generateTestTenantAlias();
//             var tenantHost = 'users.default.visibility.import.tests.com';
//             TestsUtil.createTenantWithAdmin(tenantAlias, tenantHost, function(err, tenant, tenantAdminRestContext) {
//                 assert.ok(!err);
...
//             var tenantAlias = TenantsTestUtil.generateTestTenantAlias();
//             var tenantHost = 'users.without.passwords.import.tests.com';
//             TestsUtil.createTenantWithAdmin(tenantAlias, tenantHost, function(err, tenant, tenantAdminRestContext) {
//                 assert.ok(!err);
...
//                                 assert.equal(nicolaasUserObj.email, 'nicolaas@users.without.passwords.import.tests.com');
...
//                                             assert.equal(simonUserObj.email, 'simon@users.without.passwords.import.tests.com');
...
//                                                         assert.equal(stuartUserObj.email, 'stuart@users.without.passwords.import.tests.com');
...
//          */
//         it('verify users with an email address that does not match the email domain can be imported', function(callback) {
//             var tenantAlias = TenantsTestUtil.generateTestTenantAlias();
//             var tenantHost = 'users.with.a.specific.email.domain.tests.com';
//             TestsUtil.createTenantWithAdmin(tenantAlias, tenantHost, function(err, tenant, tenantAdminRestContext) {
//                 assert.ok(!err);

//                 // The global administrator should be able to import users
//                 PrincipalsTestUtil.importUsers(globalAdminRestContext, tenant.alias, getDataFileStream('users-with-another-email-domain1.csv'), 'local', null, function(err) {
//                     assert.ok(!err);
...
//                     var userRestContext = TestsUtil.createTenantRestContext(tenantHost, 'username1', 'password');
//                     RestAPI.User.getMe(userRestContext, function(err, me) {
//                         assert.ok(!err);
...
//                         PrincipalsTestUtil.importUsers(globalAdminRestContext, tenant.alias, getDataFileStream('users-with-another-email-domain2.csv'), 'local', null, function(err) {
//                             assert.ok(!err);

//                             // Verify the user from the CSV file has been imported even though it has an email
//                             // that doesn't match the configured email domain
//                             var userRestContext = TestsUtil.createTenantRestContext(tenantHost, 'username2', 'password');
//                             RestAPI.User.getMe(userRestContext, function(err, me) {
//                                 assert.ok(!err);
...
//                     assert.equal(_.keys(retrievedUser.tenant).length, 3);
...
//          */
//         it('verify get users for tenant', function(callback) {
...
//                 assert.ok(!err);
//                 RestAPI.Admin.getAllUsersForTenant(globalAdminRestContext, global.oaeTests.tenants.cam.alias, function(err, users) {
//                     assert.ok(!err);
//                     var tenantUserIds = _.pluck(users, 'id');
...
//                     RestAPI.Admin.getAllUsersForTenant(globalAdminRestContext, 'totally-not-a-tenant-alias', function(err, users) {
//                         assert.ok(err);
//                         assert.equal(err.code, 404);
//                         // Try and get users as a non-admin user
//                         RestAPI.Admin.getAllUsersForTenant(anonymousRestContext, global.oaeTests.tenants.gt.alias, function(err, users) {
//                             assert.ok(err);
//                             assert.equal(err.code, 401);
//                             // Try and get users for a tenant with no users
//                             var tenantAlias = TenantsTestUtil.generateTestTenantAlias();
//                             var tenantHost = TenantsTestUtil.generateTestTenantHost();
//                             TenantsTestUtil.createTenantAndWait(globalAdminRestContext, tenantAlias, 'Empty tenant',  tenantHost, {},  function(err, tenant) {
//                                 assert.ok(!err);
//                                 RestAPI.Admin.getAllUsersForTenant(globalAdminRestContext, tenantAlias, function(err, users) {
//                                     assert.ok(!err);
