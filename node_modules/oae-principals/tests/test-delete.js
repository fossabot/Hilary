...
describe('Principals Delete and Restore', function() {
    ...
    describe('Profile', function() {
         ...
         * TODO
         */
        it('verify definitive delation remove user links and resources roles', function(callback) {
           // Generate a deleted user to test with
            TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users, user, otherUser, userToDelete) {
                assert.ok(!err);
                // Generate links
                PrincipalsTestUtil.generateLinks(userToDelete.restContext, 'private', function(err, link1, link2, link3) {
                    assert.ok(!err);  
                    // Generate rights links
                    PrincipalsTestUtil.generateRightContent(userToDelete, user, 'manager', link2, function(err) {
                        assert.ok(!err);
                        PrincipalsTestUtil.generateRightContent(userToDelete, otherUser, 'viewer', link2, function(err) {
                            assert.ok(!err);
                            PrincipalsTestUtil.generateRightContent(userToDelete, user, 'viewer', link3, function(err) {
                                assert.ok(!err);
                                PrincipalsTestUtil.generateRightContent(userToDelete, otherUser, 'viewer', link3, function(err) {
                                    assert.ok(!err); 
                                    // Delete User
                                    PrincipalsTestUtil.assertDefinitiveDeletionUsersSucceeds(userToDelete.restContext, userToDelete, function(err) { 
                                        // Get userToDelete
                                        PrincipalsTestUtil.assertGetUserFails(user1.restContext, user1.user.id, 404, function() {
                                            // Get links
                                            DiscussionsTestUtil.assertGetDiscussionFails(user1.restContext, link1.id, 404, function(){
                                                DiscussionsTestUtil.assertDeleteDiscussionSucceeds(user1.restContext, link2.id, function(){
                                                    DiscussionsTestUtil.assertGetDiscussionFails(user1.restContext, link3.id, 404, function(){
                                                        return callback();
                                                    });
                                                });
                                            });
         ...
         */
        it('verify definitive delation remove user files and resources roles', function(callback) {
           // Generate a deleted user to test with
            TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users, user, otherUser, userToDelete) {
                assert.ok(!err);
                // Generate files 
                PrincipalsTestUtil.generateFiles(userToDelete.restContext, 'private', function(err, file1, file2, file3) {
                    assert.ok(!err);
                    // Generate rights files
                    PrincipalsTestUtil.generateRightContent(userToDelete, user, 'manager', file2, function(err) {
                        assert.ok(!err);
                        PrincipalsTestUtil.generateRightContent(userToDelete, otherUser, 'viewer', file2, function(err) {
                            assert.ok(!err);
                            PrincipalsTestUtil.generateRightContent(userToDelete, user, 'viewer', file3, function(err) {
                                assert.ok(!err);
                                PrincipalsTestUtil.generateRightContent(userToDelete, otherUser, 'viewer', file3, function(err) {
                                    assert.ok(!err);
                                    // Delete User
                                    // TODO error here because of "camAdminRestContext"
                                    PrincipalsTestUtil.assertDefinitiveDeletionUsersSucceeds(camAdminRestContext, userToDelete, function(err) {
                                        // Get userToDelete
                                        PrincipalsTestUtil.assertGetUserFails(camAdminRestContext, user1.user.id, 404, function() {
                                            // Get links
                                            ContentTestUtil.assertGetContentFails(camAdminRestContext, file1.id, 404, function() {
                                                ContentTestUtil.assertGetContentFails(amAdminRestContext, file2.id, function() {
                                                    ContentTestUtil.assertGetContentFails(amAdminRestContext, file3.id, 404, function() {
                                                        return callback();
                                                    });
                                                });
                                            });
         ...
         */
        it('verify definitive delation remove user discussions and resources roles', function(callback) {
           // Generate a deleted user to test with
            TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users, user, otherUser, userToDelete) {
                assert.ok(!err);
                // Generate discussions
                PrincipalsTestUtil.generateDiscussions(userToDelete.restContext, 'public', function(err, disc1, disc2, disc3) {
                    assert.ok(!err);
                    // Generate rights discussions
                    PrincipalsTestUtil.generateRightDiscussion(userToDelete, user, 'manager', disc2, function(err) {
                        assert.ok(!err);
                        PrincipalsTestUtil.generateRightDiscussion(userToDelete, otherUser, 'member', disc2, function(err) {
                            assert.ok(!err);
                            PrincipalsTestUtil.generateRightDiscussion(userToDelete, user, 'member', disc3, function(err) {
                                assert.ok(!err);
                                PrincipalsTestUtil.generateRightDiscussion(userToDelete, otherUser, 'member', disc3, function(err) {
                                    assert.ok(!err);          
                                    // Delete User
                                    PrincipalsTestUtil.assertDefinitiveDeletionUsersSucceeds(userToDelete.restContext, userToDelete, function(err) {
                                        // Get userToDelete
                                        PrincipalsTestUtil.assertGetUserFails(user1.restContext, user1.user.id, 404, function() {
                                            // Get links
                                            ContentTestUtil.assertGetContentFails(user1.restContext, file1.id, 404, function(){
                                                ContentTestUtil.assertDeleteContentSucceeds(user1.restContext, file2.id, function(){
                                                    ContentTestUtil.assertGetContentFails(user1.restContext, file3.id, 404, function(){
                                                        return callback();
                                                    });
                                                });
                                            });
        ...
        /**
         * Test that verifies definitive delation remove user collabdoc and resources roles
         */
        it('verify definitive delation remove user collabdoc and resources roles', function(callback) {
           // Generate a deleted user to test with
            TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users, user, otherUser, userToDelete) {
                assert.ok(!err);
                PrincipalsTestUtil.generateCollabdocs(userToDelete.restContext, 'private', function(err, collab1, collab2, collab3, collab4) {
                    assert.ok(!err);
                    // Generate rights collabdoc
                    PrincipalsTestUtil.generateRightContent(userToDelete, user, 'manager', collab2, function(err) {
                        assert.ok(!err);
                        PrincipalsTestUtil.generateRightContent(userToDelete, otherUser, 'viewer', collab2, function(err) {
                            assert.ok(!err);
                            PrincipalsTestUtil.generateRightContent(userToDelete, user, 'editor', collab3, function(err) {
                                assert.ok(!err);
                                PrincipalsTestUtil.generateRightContent(userToDelete, otherUser, 'viewer', collab3, function(err) {
                                    assert.ok(!err);
                                    PrincipalsTestUtil.generateRightContent(userToDelete, user, 'viewer', collab4, function(err) {
                                        assert.ok(!err);
                                        PrincipalsTestUtil.generateRightContent(userToDelete, otherUser, 'viewer', collab4, function(err) {
                                            assert.ok(!err);
                                            // Delete User
                                            PrincipalsTestUtil.assertDefinitiveDeletionUsersSucceeds(userToDelete.restContext, userToDelete, function(err) {
                                                // Get userToDelete
    ...
    describe('Migrations', function() {
         ...
         */
        it('verify deleting users by tenancy', function(callback) {
             TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users, user1, user2, user3) {
                assert.ok(!err);
                PrincipalsTestUtil.assertGetUserSucceeds(user1.restContext, user1.user.id, function() {
                    PrincipalsTestUtil.assertGetUserSucceeds(user2.restContext, user2.user.id, function() {
                        PrincipalsTestUtil.assertGetUserSucceeds(user3.restContext, user3.user.id, function() {
                            var globalAdminContext = TestsUtil.createGlobalAdminContext();
                            DisableUsersMigration.doMigration(globalAdminContext, global.oaeTests.tenants.cam.alias, true, function(err, affectedUsers) {
                                assert.ok(!err);
                                // Update redis and search since we updated outside the scope of the API
                                Redis.flush(function(err) {
                                    assert.ok(!err);
                                    PrincipalsTestUtil.assertGetUserFails(user1.restContext, user1.user.id, 404, function() {
                                        PrincipalsTestUtil.assertGetUserFails(user2.restContext, user2.user.id, 404, function() {
                                            PrincipalsTestUtil.assertGetUserFails(user3.restContext, user3.user.id, 404, function() {
                                                DisableUsersMigration.doMigration(globalAdminContext, global.oaeTests.tenants.cam.alias, false, function(err, affectedUsers) {
                                                    assert.ok(!err);
                                                    Redis.flush(function(err) {
                                                        assert.ok(!err);
                                                        PrincipalsTestUtil.assertGetUserSucceeds(user1.restContext, user1.user.id, function() {
