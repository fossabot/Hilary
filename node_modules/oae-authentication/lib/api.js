    ...
    _getUserIdFromLoginId(loginId, function(err, userId) {
        if (err && err.code !== 404) {
            ...
            OaeUtil.invokeIfNecessary(opts.invitationToken, AuthzInvitationsDAO.getEmailByToken, opts.invitationToken, function(err, email) {
                ...
                OaeUtil.invokeIfNecessary(shouldCheckEmail, _validateEmailBelongsToTenant, ctx, opts.email, function(err) {
                    ...
                    createUser(ctx, loginId, displayName, opts, function(err, user) {
                        ...
                        return callback(null, user, flattenLoginId(loginId), true);
                    });
                });
            });
        } else {
            ...
            PrincipalsDAO.getPrincipal(userId, function(err, user) {
                ...
                return callback(null, user, flattenLoginId(loginId), false);
            });
        }
    });
};
...
var checkPassword = module.exports.checkPassword = function(tenantAlias, username, password, callback) {
    ...
    Cassandra.runQuery('SELECT "userId", "password" FROM "AuthenticationLoginId" WHERE "loginId" = ?', [flattenLoginId(loginId)], function(err, rows) {
        if (err) {
            return callback(err);
...
var getResetPasswordSecret = module.exports.getResetPasswordSecret = function(ctx, username, callback) {
    ...
    _getUserIdFromLoginId(loginId, function(err, userId) {
        ...
        PrincipalsDAO.getPrincipal(userId, function(err, user) {
            ...
            Cassandra.runQuery('UPDATE "AuthenticationLoginId" USING TTL 86400 SET "secret" = ? WHERE "loginId" = ?', [secret, flattenLoginId(loginId)], function(err) {
                if (err) {
...
var resetPassword = module.exports.resetPassword = function(ctx, username, secret, newPassword, callback) {
    ...
    Cassandra.runQuery('SELECT "userId", "secret" FROM "AuthenticationLoginId" WHERE "loginId" = ?', [flattenLoginId(loginId)], function(err, rows) {
...
var _changePassword = function(loginId, newPassword, callback) {
    ...
    Cassandra.runQuery('UPDATE "AuthenticationLoginId" SET "password" = ? WHERE "loginId" = ?', [hash, flattenLoginId(loginId)], function(err) {
        if (err) {
...
var _associateLoginId = function(loginId, userId, callback) {
    ...
    var flattenedLoginId = flattenLoginId(loginId);
...
var _getUserIdFromLoginId = function(loginId, callback) {
    Cassandra.runQuery('SELECT "userId" FROM "AuthenticationLoginId" WHERE "loginId" = ?', [flattenLoginId(loginId)], function(err, rows) {
        if (err) {
            return callback(err);
        } else if (_.isEmpty(rows)) {
 ...
 * @api private
 */
var flattenLoginId = function(loginId) {
...
/**
 * Expand a flattened key representation of a login id into a LoginId object. This is the opposite of #flattenLoginId.
 *
