        ...
        if(transfer.code === code && transfer.emailTarget === emailTarget) {
            ...
            Cassandra.runQuery('SELECT * FROM "AuthzRoles" WHERE "principalId" = ?', [transfer.idUserOrigin], function(err, result) {
                ...
                _getQueries(authzRoles, idUserTarget, transfer.idUserOrigin, function(err, queries, newAuthzRoles) {
                    ...
                    _executeQueries(queries, emailOrigin, idUserTarget, ctx, newAuthzRoles, code, transfer.idUserOrigin, function(err, managers) {
 ...
 */
var _getQueries = function(authzRoles, idUserTarget, idUserOrigin, callback) {
 ...
 * @api private used to make the transter
 */
var _executeQueries = function(queries, emailOrigin, idUserTarget, ctx, authzRoles, code, idUserOrigin, callback) {
    ...
    Cassandra.runBatchQuery(queries, function(err) {
        ...
        if (!authzRoles) {
            ...
            deleteTransfer(emailOrigin, code, idUserOrigin, function (err) {
                if(err) {
                    return callback(err);
                } else {
                    // Get id emailOrigin
