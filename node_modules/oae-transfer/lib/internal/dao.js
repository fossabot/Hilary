...
//var TransferConfig = ConfigAPI.config('oae-transfer');

// Library
var ContentLibrary = require('oae-content/lib/internal/membersLibrary');
var FolderLibrary = require('oae-folders/lib/internal/foldersLibrary');
            ...
            Cassandra.runQuery('SELECT * FROM "AuthzRoles" WHERE "principalId" = ?', [transfer._wrapped[0].idUserOrigin], function(err, result){
                ...
                Async.series({
                    res: function(callback){
                        _getQueries(authzRoles, idUserTarget, transfer._wrapped[0].idUserOrigin, function(err, queries, newAuthzRoles){
                            if(err){
                                callback(err);
                            }
...
_getQueries = function(authzRoles, idUserTarget, idUserOrigin, callback){
    Async.series({
        ...
        //console.log(results.res[0], results.res[1]);
        if(results.res){
...
_executeQueries = function(queries, emailOrigin, idUserTarget, ctx, authzRoles, callback){
    ...
    Cassandra.runBatchQuery(queries, function(err) {
        ...
        AuthzAPI.hasRole(idUserTarget, authzRoles[0].resourceId, AuthzConstants.role.MANAGER, function(err, targetHasRoleManager) {
        if (!authzRoles){
            ...
            deleteTransfer(emailOrigin, function (err){
                if(err){
                    ...
                    _getManagers(ctx, authzRoles, idUserTarget, function (err, managers){
                        if(err){
                            return callback(err);
                        } else {
 ...
 */
 // TODO
var _getManagers = function (ctx, authzRoles, idUserTarget, callback){
    ...
    var promise = new Promise(function(resolve, reject){
        ...
        _.chain(authzRoles).map(function(hash) { 
            ...
            Cassandra.runQuery('SELECT * FROM "AuthzMembers" WHERE "resourceId" = ?', [hash.resourceId], function(err, result){
                //console.log('_getManager - authzRoles');
                ...
                _.chain(authzMembers._wrapped).map(function(res) {
                    ...
                    if(res.role === "manager" /*&& res.memberId !== idUserTarget*/){
                        // Get the user
                        Cassandra.runQuery('SELECT * FROM "Principals" WHERE "principalId" = ?', [res.memberId], function (err, manager) {
                            ...
                            if (err) {
                                ...
                                _getNameDocument(ctx, hash.resourceId, idUserTarget, function (err, result){
                                    // TODO resolve error
...
/**
 * Get the name of the resource by it's id && take advantage of the differentiation of data type to add it to the library
 * 
 ...
 * @api private
 */
var _getNameDocument = function (ctx, idResource, principalId, callback){
    ...
    if(resourceType === "c"){     
        ContentDAO.getContent(idResource, function(err, result){
            ...
            // Add to library
            _insertOnLibrary(result, principalId, ContentConstants.library.CONTENT_LIBRARY_INDEX_NAME, function(err){
                if(err){
        ...
        _getFoldersByGroupIds(idResource, function(err, result){
            ...
            // Add to library
            _insertOnLibrary(result, principalId, FoldersConstants.library.FOLDERS_LIBRARY_INDEX_NAME, function(err){
                if(err){
                    return callback(err, result.displayName);
                }
                return callback(null, result.displayName);
        ...
        DiscussionsDAO.getDiscussion(idResource, function(err, result){
            ...
            // Add to Library
            _insertOnLibrary(result, principalId, DiscussionsConstants.library.DISCUSSIONS_LIBRARY_INDEX_NAME, function(err){
                if(err){
                    return callback(err, result.displayName);
                }
                return callback(null, result.displayName);
