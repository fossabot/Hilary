...
var makeTransfer = module.exports.makeTransfer = function (ctx, emailOrigin, code, emailTarget, idUserTarget, callback) {
    ...
    validator.check(emailTarget, {'code': 400, 'msg': 'email cannot be empty'}).notEmpty();
    ...
    TransferDAO.makeTransfer(ctx, emailOrigin, code, emailTarget, idUserTarget, function(err, managers){
        if(err){
            ...
            sendEmail(ctx, managers, emailOrigin, emailTarget, function(err, managers){
                if(err){
                    return callback(err);
                }
            });
 ...
 * @api private
 */
var sendEmail = module.exports.sendEmail = function(ctx, managers, oldUserMail, newUserMail, callback) {
    ...
    managers.map(function(hash){
	    ...
	    PrincipalsDAO.storeEmailToken(ctx.user().id, hash.email, token, function(err) {
            ...
            console.log("mail envoyé à " + hash.email + "   " + hash.displayName + "    " + hash.resource);
	        return callback(null);
	    });
    });
};
