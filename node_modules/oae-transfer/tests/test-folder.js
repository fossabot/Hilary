...
describe('Transfer-folder', function() {
    ...
    describe('Make transfer - folder', function() {
        /**
        ...
        it('verify if transfer find good manager', function(callback) {
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users, mrvisser, stuartf, userOrigin) {
                ...
                TestsUtil.generateTestUsers(gtAdminRestContext, 1, function(err, users, userTarget) {
                    assert.ok(!err);
                    // Generate folder
                    FoldersTestUtil.assertCreateFolderSucceeds(mrvisser.restContext, 'test displayName', 'test description', 'private', [mrvisser, userOrigin], [stuartf], function(createdFolder) {
        ...
        it('verify if transfer give good right', function(callback) {
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, mrvisser, userOrigin) {
                ...
                TestsUtil.generateTestUsers(gtAdminRestContext, 1, function(err, users, userTarget) {
                    assert.ok(!err);
                    // Generate folder
                    FoldersTestUtil.assertCreateFolderSucceeds(mrvisser.restContext, 'test displayName', 'test description', 'private', [mrvisser, userOrigin], [], function(createdFolder) {
                        assert.ok(createdFolder);
         ...
         */
        it('verify the case A transfer to B, A is manager and B is viewer : the role will be update', function(callback) {
            // Generate user in cam tenant
            TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, mrvisser, userOrigin) {
                assert.ok(!err);
                // Generate user in gt tenant
                TestsUtil.generateTestUsers(gtAdminRestContext, 1, function(err, users, userTarget) {
                    assert.ok(!err);
                    // Generate folder
                    FoldersTestUtil.assertCreateFolderSucceeds(mrvisser.restContext, 'test displayName', 'test description', 'private', [mrvisser, userOrigin], [userTarget], function(createdFolder) {
                        assert.ok(createdFolder);
                        TransferTestUtil.assertCreateTransferSucceeds(userOrigin.restContext, userOrigin.user.email, userTarget.user.email, userOrigin.user.id, function(transfer) {
                            assert.ok(transfer);
                            TransferTestUtil.assertMakeTransferSucceeds(userOrigin.restContext, userOrigin.user.email, transfer.code, userTarget.user.email, userTarget.user.id, function(err){
         ...
         */
        it('verify the case A transfer to B, A the viewer and B is the manager : the role will not be update', function(callback) {
            // Generate user in cam tenant
            TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, mrvisser, userOrigin) {
                assert.ok(!err);
                // Generate user in gt tenant
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, mrvisser, userOrigin) {
                ...
                TestsUtil.generateTestUsers(gtAdminRestContext, 1, function(err, users, userTarget) {
                    ...
                    FoldersTestUtil.assertCreateFolderSucceeds(mrvisser.restContext, 'test displayName', 'test description', 'private', [userTarget], [userOrigin], function(createdFolder) {
                        assert.ok(createdFolder);
                        // Create file
                        TransferTestUtil.generateFile(mrvisser.restContext, 'private', function(err, mrvisserfile1){
                            assert.ok(!err);
                            TransferTestUtil.generateFile(mrvisser.restContext, 'loggedin', function(err, mrvisserfile2){
                                assert.ok(!err);
                                TransferTestUtil.generateFile(mrvisser.restContext, 'public', function(err, mrvisserfile3){
                                    assert.ok(!err);
                                    FolderAPI.addContentItemsToFolder(mrvisser.restContext, createdFolder.id, [mrvisserfile1.id, mrvisserfile2.id, mrvisserfile3.id], function(err){
                                        assert.ok(!err);
                                        TransferTestUtil.assertCreateTransferSucceeds(userOrigin.restContext, userOrigin.user.email, userTarget.user.email, userOrigin.user.id, function(transfer) {
                                            assert.ok(transfer);
                                            // Verify the role
                                                            ...
                                                            FoldersTestUtil.assertCreateFolderSucceeds(orodan.restContext, 'test displayName', 'test description', 'private', [], [userTarget], function(orodanfolder2) {
                                                                ...
                                                                FoldersTestUtil.assertCreateFolderSucceeds(orodan.restContext, 'test displayName', 'test description', 'private', [], [], function(orodanfolder3) {
                                                                    ...
                                                                    RestAPI.Folders.getFoldersLibrary(userOrigin.restContext, userOrigin.user.id, null, null, function(err, result) {
                                                                        var listFoldersBefore = [];
                                                                        result.results.forEach(function(i){
                                                                            listFoldersBefore.push(i.id);
                                                                        });
                                                                        RestAPI.Folders.getFoldersLibrary(userTarget.restContext, userTarget.user.id, null, null, function(err, result) {
                                                                            ...
                                                                            TransferTestUtil.assertCreateTransferSucceeds(userOrigin.restContext, userOrigin.user.email, userTarget.user.email, userOrigin.user.id, function(transfer) {
                                                                                ...
                                                                                TransferTestUtil.assertMakeTransferSucceeds(userOrigin.restContext, userOrigin.user.email, transfer.code, userTarget.user.email, userTarget.user.id, function(err, managers){
                                                                                    ...
                                                                                    RestAPI.Folders.getFoldersLibrary(userOrigin.restContext, userOrigin.user.id, null, null, function(err, result) {
                                                                                        RestAPI.Folders.getFoldersLibrary(userTarget.restContext, userTarget.user.id, null, null, function(err, result) {
                                                                                            var listFoldersTargetAfter = [];
                                                                                            result.results.forEach(function(i){
                                                                                                listFoldersTargetAfter.push(i.id);
                                                                                            });
