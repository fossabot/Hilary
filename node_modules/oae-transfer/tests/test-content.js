...
describe('Transfer-content', function() {
    ...
    /**
     * Test that verify if transfer works for collabdocs
     */
     /*
        ...
        it('verify if transfer find good manager', function(callback) {
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 3, function(err, users, mrvisser, stuartf, userOrigin) {
                ...
                TestsUtil.generateTestUsers(gtAdminRestContext, 1, function(err, users, userTarget) {
                    assert.ok(!err);
                    // Generate collabdocs
                    TransferTestUtil.generateCollabdoc(mrvisser, 'private', function(err, mrvissercollab){
                        assert.ok(!err);
                        // Generate rights
        ...
        it('verify the case A transfer to B, A the member and B is the owner : the role will not be update', function(callback) {
            ...
            TestsUtil.generateTestUsers(camAdminRestContext, 2, function(err, users, mrvisser, userOrigin) {
                ...
                TestsUtil.generateTestUsers(gtAdminRestContext, 1, function(err, users, userTarget) {
                    assert.ok(!err);
                    // Generate collabdocs
                    TransferTestUtil.generateCollabdoc(userTarget.restContext, 'private', function(err, mrvissercollab){
                        assert.ok(!err);
                        // Generate rights
                        TransferTestUtil.generateRightContent(userTarget, userOrigin, 'viewer', mrvissercollab, function(err){
                            assert.ok(!err);
                            TransferTestUtil.assertCreateTransferSucceeds(userOrigin.restContext, userOrigin.user.email, userTarget.user.email, userOrigin.user.id, function(transfer) {
                                assert.ok(transfer);
                                TransferTestUtil.assertMakeTransferFails(userOrigin.restContext, userOrigin.user.email, transfer.code, userTarget.user.email, userTarget.user.id, 405, function(err){
