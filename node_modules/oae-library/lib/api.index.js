...
var list = module.exports.list = function(indexName, libraryId, visibility, opts, callback) {
    ...
    _query(indexName, libraryId, visibility, {'start': opts.start, 'limit': opts.limit, 'rebuildIfNecessary': true}, function(err, rawEntries, nextToken) {
        if (err) {
            return callback(err);
        }
        ...
        var resourceIds = {};
        var entries = [];
        _.each(rawEntries, function(rawEntry) {
            numResults++;
...
var _adjustColumnsForSlugs = function(indexName, libraryId, start, limit, rows, nextToken) {
    ...
    _.each(rows, function(row) {
        ...
        try {
            value = JSON.parse(row.get('value').value);
        } catch (err) {
            log().error({
                'err': err,
                'indexName': indexName,
